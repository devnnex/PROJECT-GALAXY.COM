...
El SQL crea el bucket privado `premium-downloads`, el producto y la auditoría de descargas. El Scanner aparece en el catálogo de todas las cuentas activas con la imagen `src/assets/PowerElite.png`, precio promocional recurrente de 650 USDT frente al precio anterior de 1000 USDT y un contador cuyo ciclo se renueva cada 24 horas de forma explícita. Las cuentas distintas de `elkin56ty@gmail.com` pueden abrirlo y continuar por el checkout manual normal, pero no reciben el botón de descarga. `scanner-download` repite la validación del correo propietario en el servidor. El archivo fuente está ignorado por Git para que GitHub Pages nunca lo publique.
