...
create table if not exists public.platform_reports (
  ...
  subject text not null check (char_length(subject) between 4 and 140), details text not null check (char_length(details) between 10 and 3000),
  priority text not null default 'NORMAL' check (priority in ('LOW','NORMAL','HIGH','CRITICAL')),
  status text not null default 'OPEN' check (status in ('OPEN','IN_REVIEW','RESOLVED')),
  admin_notes text not null default '' check (char_length(admin_notes) <= 2000), resolved_at timestamptz,
...
create trigger meeting_music_tracks_touch_updated_at before update on public.meeting_music_tracks for each row execute function public.touch_updated_at();
drop trigger if exists platform_reports_touch_updated_at on public.platform_reports;
create trigger platform_reports_touch_updated_at before update on public.platform_reports for each row execute function public.touch_updated_at();
...
begin
  ...
  if coalesce(p_storage_path,'') !~ '^[0-9a-f-]{36}/[0-9a-f-]{36}\.(mp3|m4a|aac|ogg|weba|wav|flac)$'
    or split_part(p_storage_path,'/',1)<>p_id::text then raise exception 'La ruta del audio no es valida.' using errcode='P0001'; end if;
...
language sql stable security definer set search_path=public,auth as $$
  select exists(select 1 from auth.users account join public.profiles profile on profile.id=account.id
    where account.id=auth.uid() and lower(account.email)='elkin56ty@gmail.com' and profile.status='ACTIVE');
$$;
...
language sql stable security definer set search_path=public as $$
  select jsonb_build_object('id',p_report.id,'reporterId',p_report.reporter_id,'reporterName',profile.name,
    'reporterUsername',profile.username,'reporterAvatar',profile.avatar,'reporterMembership',public.membership_view(profile.id),
    'category',p_report.category,'subject',p_report.subject,'details',p_report.details,'priority',p_report.priority,
...
begin
  if coalesce(p_category,'') not in ('PLATFORM','MEETING','AUDIO','ACCOUNT','PAYMENT','OTHER') then raise exception 'Selecciona una categoría válida.' using errcode='P0001'; end if;
  if char_length(trim(coalesce(p_subject,''))) not between 4 and 140 then raise exception 'El asunto debe tener entre 4 y 140 caracteres.' using errcode='P0001'; end if;
  if char_length(trim(coalesce(p_details,''))) not between 10 and 3000 then raise exception 'Los detalles deben tener entre 10 y 3000 caracteres.' using errcode='P0001'; end if;
  if (select count(*) from public.platform_reports where reporter_id=v_user and created_at>now()-interval '1 hour')>=10 then raise exception 'Alcanzaste el límite temporal de reportes. Intenta nuevamente más tarde.' using errcode='P0001'; end if;
  insert into public.platform_reports(reporter_id,category,subject,details) values(v_user,p_category,trim(p_subject),trim(p_details)) returning * into v_report;
...
begin
  select coalesce(jsonb_agg(public.platform_report_view(report) order by
    ...
    case report.priority when 'CRITICAL' then 0 when 'HIGH' then 1 when 'NORMAL' then 2 else 3 end,report.created_at desc),'[]'::jsonb)
  into v_result from public.platform_reports report where v_controller or report.reporter_id=v_user;
  return v_result;
end; $$;
...
begin
  ...
  if coalesce(p_priority,'') not in ('LOW','NORMAL','HIGH','CRITICAL') or coalesce(p_status,'') not in ('OPEN','IN_REVIEW','RESOLVED') then raise exception 'La clasificación del reporte no es válida.' using errcode='P0001'; end if;
  ...
  update public.platform_reports set priority=p_priority,status=p_status,admin_notes=trim(coalesce(p_admin_notes,'')),resolved_at=case when p_status='RESOLVED' then coalesce(resolved_at,now()) else null end where id=p_report_id returning * into v_report;
...
language plpgsql security definer set search_path=public,auth as $$
begin perform public.require_registered_member(); if not public.is_platform_reports_controller() then raise exception 'Solo Elkin puede eliminar reportes.' using errcode='P0001'; end if; delete from public.platform_reports where id=p_report_id; return found; end; $$;

create or replace function public.clear_platform_reports() returns integer
language plpgsql security definer set search_path=public,auth as $$
declare v_count integer;
begin perform public.require_registered_member(); if not public.is_platform_reports_controller() then raise exception 'Solo Elkin puede vaciar los reportes.' using errcode='P0001'; end if; delete from public.platform_reports; get diagnostics v_count=row_count; return v_count; end; $$;

create or replace function public.direct_messages_controller_id() returns uuid
language sql stable security definer set search_path=public,auth as $$
  select account.id from auth.users account join public.profiles profile on profile.id=account.id
  where lower(account.email)='elkin56ty@gmail.com' and profile.status='ACTIVE' and not profile.is_guest limit 1;
$$;
...
language plpgsql stable security definer set search_path=public,auth as $$
declare v_user uuid:=auth.uid(); v_left_text text:=split_part(coalesce(p_topic,''),':',2); v_right_text text:=split_part(coalesce(p_topic,''),':',3); v_left uuid; v_right uuid;
begin
  if coalesce(p_topic,'') !~ '^direct:[0-9a-f-]{36}:[0-9a-f-]{36}$' or split_part(p_topic,':',4)<>'' then return false; end if;
  begin v_left:=v_left_text::uuid; v_right:=v_right_text::uuid; exception when invalid_text_representation then return false; end;
  return v_user in (v_left,v_right) and v_left_text<v_right_text and public.can_direct_message(v_left,v_right);
end; $$;
...
begin
  ...
  if (select count(*) from public.direct_messages where sender_id=v_user and created_at>now()-interval '10 seconds')>=20 then raise exception 'Estás enviando mensajes demasiado rápido.' using errcode='P0001'; end if;
...
begin
  ...
  update public.direct_messages set delivered_at=coalesce(delivered_at,now()),read_at=coalesce(read_at,now()) where sender_id=p_sender_id and recipient_id=v_user and read_at is null;
...
create policy meeting_music_tracks_authenticated_read on public.meeting_music_tracks for select to authenticated using (
  active and public.is_current_session_valid()
);
drop policy if exists platform_reports_member_read on public.platform_reports;
create policy platform_reports_member_read on public.platform_reports for select to authenticated using (
  public.is_current_session_valid() and (reporter_id=auth.uid() or public.is_platform_reports_controller())
);
...
revoke all on function public.is_platform_reports_controller(),public.require_registered_member(),public.platform_report_view(public.platform_reports),
  public.create_platform_report(text,text,text),public.get_platform_reports(),public.update_platform_report(uuid,text,text,text),
...
grant execute on function public.is_platform_reports_controller(),public.create_platform_report(text,text,text),public.get_platform_reports(),
  ...
  public.get_direct_message_contacts(),public.get_direct_messages(uuid,integer),public.send_direct_message(uuid,text),
  public.mark_direct_messages_read(uuid),public.mark_direct_messages_delivered(),public.can_direct_message(uuid,uuid) to authenticated;
