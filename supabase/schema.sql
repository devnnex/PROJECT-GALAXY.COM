...
declare v_user uuid:=public.require_registered_member(); v_result jsonb;
begin
  if p_user_id=v_user or not exists(select 1 from public.profiles where id=p_user_id and status='ACTIVE' and not is_guest and public.has_active_membership(id)) then raise exception 'La conversación no está disponible.' using errcode='P0001'; end if;
...
begin
  if p_recipient_id=v_user or not exists(select 1 from public.profiles where id=p_recipient_id and status='ACTIVE' and not is_guest and public.has_active_membership(id)) then raise exception 'No puedes enviar mensajes a esta cuenta.' using errcode='P0001'; end if;
...
begin update public.direct_messages set read_at=coalesce(read_at,now()) where sender_id=p_sender_id and recipient_id=v_user and read_at is null; get diagnostics v_count=row_count; return v_count; end; $$;
...
revoke all on function public.is_platform_reports_controller(),public.require_registered_member(),public.platform_report_view(public.platform_reports),
  ...
  public.mark_direct_messages_read(uuid) from public,anon,authenticated;
