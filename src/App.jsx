...
import { MembershipActivationModal, MembershipCheckoutModal, MembershipGate, MembershipOrdersPage, MembershipProfileCard } from './components/MembershipExperience';
...
function Landing({ onEnter }) {
  return <main className="landing">
    ...
    <section className="hero">
      ...
      <p>Un espacio privado para estudiar y operar el oro digital (XAUUSD), identificar zonas de liquidez y preparar cada sesión con contexto.</p>
    ...
    <section className="ecosystem-section" id="ecosystem"><p className="eyebrow">THE XAUUSD WORKFLOW</p><h2>Lectura, contexto y disciplina en una misma sala.</h2><div className="ecosystem-grid">{[['SCAN', Users, 'Observa estructura, sesiones y contexto de XAUUSD.'], ['MAP', Sparkles, 'Marca pools de liquidez, rangos y niveles de interés.'], ['REPLAY', Radio, 'Revisa la narrativa del precio antes y después de cada sesión.'], ['MEET', Video, 'Comparte análisis de London y New York en tiempo real.'], ['JOURNAL', TrendingUp, 'Documenta hipótesis, ejecución y aprendizaje operativo.'], ['EXECUTE', CircleDollarSign, 'Decisiones con gestión de riesgo; nunca promesas de rentabilidad.']].map(([name, Icon, text], i) => <article key={name}><span>0{i + 1}</span><Icon /><h3>{name}</h3><p>{text}</p></article>)}</div></section>
...
function Dashboard({ user, navigate, openProduct }) {
  ...
  return <div className="page-stack">
    <header className="page-header"><div><p className="eyebrow">XAUUSD COMMAND CENTER</p><h1>Bienvenido de vuelta, {user.name.split(' ')[0]}.</h1><p>Prepara tu lectura de liquidez para la próxima Kill Zone.</p></div><button className="primary-button compact" onClick={() => navigate('discover')}><Plus /> Analizar</button></header>
    ...
    <div className="dashboard-grid"><section className="surface activity-panel"><div className="section-title"><div><p className="eyebrow">MARKET CONTEXT</p><h2>Bitácora de sesión</h2></div></div><EmptyState icon={Activity} title="Sin análisis registrados" text="Aquí aparecerán tus lecturas de estructura, liquidez y sesiones de XAUUSD." /></section><section className="surface next-meeting"><div className="section-title"><div><p className="eyebrow">KILL ZONE</p><h2>Sala de análisis</h2></div><Video /></div><div className="meeting-art"><span className="pulse-ring" /><Orbit /></div><h3>Sesiones privadas</h3><p>Comparte contexto de XAUUSD antes de London o New York.</p><button className="primary-button" onClick={() => navigate('meetings')}>Abrir sesiones <ArrowRight /></button></section></div>
    <section><div className="section-title"><div><p className="eyebrow">TRADING TOOLKIT</p><h2>Herramientas para XAUUSD</h2></div><button className="text-button" onClick={() => navigate('marketplace')}>Marketplace <ArrowRight /></button></div><ProductGrid items={products.slice(0, 3)} onOpen={openProduct} /></section>
  </div>;
}
...
function MembershipMarketplaceCards({ product, onOpen }) {
  return <section className="membership-marketplace surface">
    <header><div><p className="eyebrow">PROJECT GALAXY XAUUSD MEMBERSHIP</p><h2>Activa tu acceso a las Kill Zones.</h2><p>Sesiones privadas de análisis XAUUSD, LIVE, chat y pantalla compartida en una sola membresía.</p></div><span><ShieldCheck /> USDT · TRC20 / ERC20</span></header>
...
function LivePage({ toast }) { const [following, setFollowing] = useState(false); return <div className="page-stack"><header className="page-header"><div><p className="eyebrow">XAUUSD LIVE DESK</p><h1>Programación de sesiones</h1><p>Seguimiento educativo de Kill Zones, estructura y liquidez.</p></div><button className="primary-button compact" onClick={() => toast('Para transmitir a una audiencia configura el servicio SFU, TURN y señalización.', 'info')}><Radio /> Iniciar sesión</button></header><div className="live-feature surface"><div className="live-art"><div className="live-badge">VISTA PREVIA</div><button className="play-button" onClick={() => toast('Esta pieza visual no representa una transmisión conectada.', 'info')}><Play /></button></div><div><span className="post-tag">XAUUSD · LIQUIDEZ</span><h2>Preparación de la Kill Zone de New York</h2><p>Observa cómo se organiza el contexto entre rango asiático, barridos de liquidez y apertura de sesión.</p><div className="person"><div className="avatar">MC</div><span>Galaxy Trading Desk</span><button onClick={() => setFollowing(!following)}>{following ? 'Siguiendo' : 'Seguir'}</button></div></div></div><div className="product-grid">{products.slice(0, 3).map((p, i) => <article className="stream-card surface" key={p.id}><div className={`stream-art ${p.tone}`}><span className="live-badge">PROGRAMADO</span><Radio /></div><span>Mañana · {14 + i}:00</span><h3>{['Contexto de Asia y Londres', 'Pools de liquidez en XAUUSD', 'Plan de ejecución para New York'][i]}</h3><p>{p.seller}</p></article>)}</div></div>; }
...
function AppShell({ user, onUserChange, onLogout }) {
  ...
  const [membershipCenter, setMembershipCenter] = useState({ membership: user.membership || { isActive: false }, plans: [], orders: [] }); const [activatedMembership, setActivatedMembership] = useState(null); const [membershipClock, setMembershipClock] = useState(Date.now());
  ...
  const membershipActivated = async (confirmed) => {
    ...
    try { const [center, refreshedUser] = await Promise.all([reloadMembership(), api.me()]); if (refreshedUser) onUserChange(refreshedUser); if (center?.membership) setActivatedMembership(center.membership); } catch {}
  };
  ...
  else if (page === 'live') content = membershipActive ? <LivePage toast={toast} /> : <MembershipGate section="LIVE" onChoosePlan={openMembership} />;
  else if (page === 'meetings') content = membershipActive ? <MeetingStudio toast={toast} user={user} joinRequest={joinRequest} /> : <MembershipGate section="Reuniones" onChoosePlan={openMembership} />;
  ...
  return <div className="app-shell">
    ...
    <ProductModal product={selectedProduct} onClose={() => setSelectedProduct(null)} toast={toast} membershipCenter={membershipCenter} onMembershipActivated={membershipActivated} />
    <MembershipActivationModal membership={activatedMembership} onClose={() => setActivatedMembership(null)} onOpenSessions={() => { setActivatedMembership(null); navigate('meetings'); }} />
