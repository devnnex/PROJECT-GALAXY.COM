...
const catalogFor = (user) => products.filter((product) => product.kind !== 'scanner' || isScannerOwner(user));
...
function ProductModal({ product, onClose, toast, membershipCenter, user }) {
  ...
  if (product.kind === 'scanner') return isScannerOwner(user) ? <ScannerCheckoutModal onClose={onClose} toast={toast} /> : null;
  const createPayment = () => {
