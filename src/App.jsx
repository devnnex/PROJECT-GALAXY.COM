...
const isMeetingOwner = (user) => String(user?.email || '').trim().toLowerCase() === 'elkin56ty@gmail.com';
...
function MembershipMarketplaceCards({ product, onOpen }) {
  return <section className="membership-marketplace surface">
    <header><div><p className="eyebrow">PROJECT GALAXY XAUUSD</p><h2>Accede a una experiencia de trading de alto valor.</h2><p>Elige el plan que mejor se adapte a tu operativa. El pago se realiza directamente y se confirma manualmente.</p></div><span><ShieldCheck /> USDT · TRC20 / ERC20</span></header>
...
function ProfileEditor({ user, onClose, onSaved }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="profile-editor glass" role="dialog" aria-modal="true" aria-labelledby="profile-editor-title" onMouseDown={(event) => event.stopPropagation()}>
    ...
    <form onSubmit={submit}>
      ...
      <label className="profile-language-field"><span><Languages /> Idioma de la plataforma</span><select name="language" value={form.language} onChange={update}><option value="es">Español</option><option value="en">English</option></select><small>La navegación, tu wallet y las notificaciones usarán este idioma.</small></label>
...
function AppShell({ user, onUserChange, onLogout }) {
  ...
  const membership = membershipForAvatar({ ...user, membership: membershipCenter.membership || user.membership });
  ...
  const visibleNotifications = notificationFilter === 'UNREAD' ? notificationItems.filter((notice) => !notice.readAt) : notificationItems;
...
export default function App() {
  ...
  const [view, setView] = useState(sharedIntent || new URLSearchParams(location.hash.slice(1)).has('registration') ? 'auth' : 'landing'); const [user, setUser] = useState(null); const [ready, setReady] = useState(false); const [sessionError, setSessionError] = useState('');
  useEffect(() => {
