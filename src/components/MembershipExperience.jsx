...
export function ScannerCheckoutModal({ product, onClose, toast }) {
  ...
  return <div className="modal-backdrop membership-modal-backdrop" onMouseDown={onClose}><section className="membership-checkout glass" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
    <button className="icon-button modal-close" onClick={onClose} aria-label="Cerrar"><X /></button>
    <header>{product?.image ? <img className="scanner-checkout-image" src={product.image} alt={product.title} /> : <span className="membership-checkout-mark"><Sparkles /></span>}<div><p className="eyebrow">PRIVATE OWNER TOOL</p><h1>Scanner Power Elite</h1><p>Este recurso privado está disponible únicamente para la cuenta propietaria.</p></div></header>
