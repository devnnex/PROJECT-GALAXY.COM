...
function VideoSurface({ stream, name, avatarSeed, muted = false, playAudio = true, speaking = false, handRaised = false, presentation = false }) {
...
function InvitePanel({ members, onInvite, onClose }) {
  return <div className="modal-backdrop"><div className="invite-modal glass"><div className="modal-title"><div><p className="eyebrow">COMUNIDAD</p><h2>Invitar usuarios registrados</h2></div><button className="icon-button" onClick={onClose}><X /></button></div><div className="invite-list">{members.map((member) => {
    const accepted = member.participantStatus === 'ADMITTED' || member.invitationStatus === 'ACCEPTED';
    const pending = member.invitationStatus === 'PENDING'; const declined = member.invitationStatus === 'DECLINED';
    const statusText = accepted ? 'Aceptó la invitación' : pending ? member.invitationSeenAt ? 'Vio el modal · aún no responde' : 'Modal pendiente de ver' : declined ? 'Rechazó la invitación' : 'Disponible para invitar';
    return <div className="person" key={member.id}><ConstellationAvatar className="avatar avatar-sm" seed={member.id} name={member.name} /><span><strong>{member.name}</strong><small>@{member.username}</small><em className={`invite-state ${accepted ? 'accepted' : pending ? 'pending' : declined ? 'declined' : ''}`}>{statusText}</em></span><button className="secondary-button" disabled={accepted || pending} onClick={() => onInvite(member)}>{accepted ? <><Check /> Dentro</> : pending ? <><Check /> Esperando</> : declined ? <><UserPlus /> Reinvitar</> : <><UserPlus /> Invitar</>}</button></div>;
...
export default function MeetingStudio({ toast, user, joinRequest, onSessionChange }) {
  ...
  const [sideTab, setSideTab] = useState('people'); const [messages, setMessages] = useState([]); const [replyTo, setReplyTo] = useState(null); const [inviteOpen, setInviteOpen] = useState(false); const [members, setMembers] = useState([]);
  ...
  return <section className="meeting-page">
    ...
    <div className="meeting-grid">
      <div className="meeting-stage">
        {presentationStream ? <><VideoSurface presentation stream={presentationStream} name={sharing ? 'Tu pantalla' : `${presentationPeer?.name || 'Participante'} · pantalla`} avatarSeed={sharing ? user.id : presentationPeer?.userId} muted playAudio={false} /><span className="presenter-label">{sharing ? 'Tu pantalla · compartiendo por WebRTC' : `${presentationPeer?.name || 'Participante'} está compartiendo`}</span><CollaborationOverlay active={canCollaborate && Boolean(collaborationMode)} mode={collaborationMode} color={collaborationColor} strokes={annotationStrokes} cursors={remoteCursors} onPoint={sendCollaborationPoint} /></> : <div className="video-grid"><VideoSurface stream={media} name={`${user.name} · Tú`} avatarSeed={user.id} muted speaking={localSpeaking} handRaised={handRaised} />{remoteEntries.map(([peerId, stream]) => { const peer = participants.find((item) => item.peerId === peerId); return <VideoSurface key={peerId} stream={stream} name={peer?.name || 'Participante'} avatarSeed={peer?.userId || peerId} playAudio={false} speaking={peer?.speaking} handRaised={peer?.handRaised} />; })}</div>}
