...
const COSMIC_REACTIONS = [
  ...
  { id: 'UFO', label: 'McLaren Profit', asset: MCLAREN_PROFIT_ASSET },
...
function CosmicReaction({ reaction, senderName }) {
  ...
  if (reaction === 'UFO') return <span className="mclaren-profit-reaction" role="img" aria-label="McLaren llegando, lanzando billetes y celebrando Profit"><img className="mclaren-profit-car" src={MCLAREN_PROFIT_ASSET} alt="" /><span className="mclaren-profit-bills" aria-hidden="true">{Array.from({ length: 18 }, (_, index) => <i key={index} style={{ '--bill-x': `${((index * 59) % 420) - 210}px`, '--bill-rotate': `${((index * 47) % 150) - 75}deg`, '--bill-delay': `${2.25 + (index % 6) * .12}s`, '--bill-duration': `${2.28 + (index % 4) * .22}s` }} />)}</span><span className="mclaren-profit-firework" aria-hidden="true"><i /><b /><strong>PROFIT</strong></span>{sender}</span>;
