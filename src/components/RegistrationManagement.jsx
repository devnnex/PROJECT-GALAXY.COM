...
export function WalletActivity({ user }) {
  ...
  return <div className="page-stack"><header className="page-header"><div><p className="eyebrow">WALLET</p><h1>Wallet</h1><p>Ingresos de membresías y comisiones por referidos. Se actualiza cada 15 segundos.</p></div></header>
    ...
    <section className="surface registration-panel"><h2>Movimientos y vencimientos</h2>{error && <p role="alert">{error}</p>}{!data && !error && <p>Cargando…</p>}{data?.entries.length === 0 && <p>Aún no hay movimientos de membresías.</p>}
      {data?.entries.map(entry => <article className="membership-ledger-row" key={entry.id}><div><strong>{entryLabel(entry.kind)}</strong><p>{entry.memberName} · {MEMBERSHIP_EMOJI[entry.planCode]} {entry.planCode}</p><small>{date(entry.createdAt)} · Valor de membresía: {amount(entry.grossAmount)} USDT</small><p>{entry.memberDeleted ? 'Cuenta eliminada · vencimiento histórico: ' : new Date(entry.expiresAt) <= new Date() ? 'Membresía vencida: ' : 'Membresía vigente hasta: '}{date(entry.expiresAt)}</p></div><strong>{Number(entry.amount) >= 0 ? '+' : ''}{amount(entry.amount)} USDT</strong></article>)}
    </section></div>;
}
