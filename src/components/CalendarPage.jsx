  ...
  return <div className="calendar-page">
    ...
    <div className="calendar-layout">
      <section className="calendar-board surface">
        ...
        <div className="calendar-grid">{days.map((day) => { const key = dateKey(day); const items = byDay[key] || []; const isToday = key === dateKey(new Date()); const outside = day.getMonth() !== month.getMonth(); return <div className={`calendar-cell ${outside ? 'outside' : ''}`} key={key}><span className={isToday ? 'today' : ''}>{day.getDate()}</span><div>{items.slice(0, 3).map((event) => { const state = eventState(event, clock); const eventTime = dateLabel(event.startsAt, { hour: '2-digit', minute: '2-digit' }); return <button className={`calendar-event-chip ${state.toLowerCase()} ${event.kind.toLowerCase()}`} key={event.id} onClick={() => setSelected(event)} title={event.title} aria-label={`${eventTime} ${event.title}`}><i />{eventTime} {event.title}</button>; })}{items.length > 3 && <button className="calendar-more" onClick={() => setSelected(items[3])}>+{items.length - 3} más</button>}</div></div>; })}</div>
      </section>
