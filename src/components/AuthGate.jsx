...
export default function AuthGate({ onAuthenticated, onBack }) {
  ...
  const [canResumeMeeting] = useState(() => { try { return Object.keys(localStorage).some((key) => /^galaxy_(active|resume)_meeting_/.test(key)); } catch { return false; } });
  ...
  return <main className="auth-page">
    ...
    <section className="auth-card glass">
      ...
      {token && mode === 'register' && <div className="mode-note">{invitation ? seconds > 0 ? (invitation.userKind === 'NEW' ? 'Invitación gratuita a reunión' : invitation.plan_code) + ' · Invitación: ' + Math.floor(seconds / 60) + ':' + String(seconds % 60).padStart(2, '0') : 'La invitación venció. Solicita una nueva al administrador.' : error ? 'No se pudo validar esta invitación.' : 'Validando invitación…'}</div>}
      ...
      <form onSubmit={submit}>
        ...
        <button className="primary-button auth-submit" disabled={busy || (mode === 'register' && (!invitation || seconds <= 0))}>{busy ? <LoaderCircle className="spin" /> : <>{mode === 'login' ? canResumeMeeting ? 'Volver a la sala' : 'Entrar al trading desk' : 'Crear acceso'} <ArrowRight size={18} /></>}</button>
      </form>
