...
export class SupabaseMeetingConnection extends MeetingConnection {
  ...
  async connect({ roomId, stream, iceServers = [], role = 'PARTICIPANT', hostId, user }) {
    ...
    this.identity = { peerId: this.selfId, userId: identity.id, name: identity.name, username: identity.username || '', avatar: identity.avatar || '', membership: identity.membership, role, connectedAt: Date.now(), ...(this.presence || {}) };
